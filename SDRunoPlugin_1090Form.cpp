...
// Form deconstructor
	SDRunoPlugin_1090Form::~SDRunoPlugin_1090Form () {
}

// Start Form and start Nana UI processing
void	SDRunoPlugin_1090Form::Run () {
	show ();
	nana::exec();
}

// Create the initial plugin form
void	SDRunoPlugin_1090Form::Setup () {
	...
	hModule = GetModuleHandle(L"SDRunoPlugin_1090");
	hdc = GetDC(NULL);
	...
	*(BITMAPINFO*)(settoverPixels + sizeof(BITMAPFILEHEADER)) = bmInfo_sett_over;
	...
	title_bar_label.caption("< bold size = 6 color = 0x000000 font = \"Verdana\">1090</>");
	...
	correction. push_back ("no correction");
	correction. push_back ("1 bit correction");
	correction. push_back ("2 bits correction");
	correction. option (0);
        correction. events(). selected ([&](const nana::arg_combox &ar_cbx){
                        set_correction (ar_cbx. widget. caption ());});

	displayMode. push_back ("interactive");
	displayMode. push_back ("single planes");
	displayMode. option (0);
	displayMode. events(). selected ([&](const nana::arg_combox &ar_cbx){
                        set_displayMode (ar_cbx. widget. caption ());});

	metricsMode. push_back ("metric");
	metricsMode. push_back ("non metric");
	metricsMode. option (0);
	metricsMode. events(). selected ([&](const nana::arg_combox &ar_cbx){
                        set_metricsMode (ar_cbx. widget. caption ());});

	validPreambles. transparent (true);
	validPreambles. fgcolor (nana::colors::white);
	validPreambles. tooltip ("nr of valid preambles found");

	goodCrc. transparent (true);
	goodCrc. fgcolor (nana::colors::white);
	goodCrc. tooltip ("nr of correct CRC's");

	nrfixed. transparent (true);
	nrfixed. fgcolor (nana::colors::white);
	nrfixed. tooltip ("nr of successful corrections");

	single_bit_fixed. transparent (true);
	single_bit_fixed. fgcolor (nana::colors::white);
	single_bit_fixed. tooltip ("nr of single bit corrections");

	two_bit_fixed. transparent (true);
	two_bit_fixed. fgcolor (nana::colors::white);
	two_bit_fixed. tooltip ("ne of two bit corrections");
...
void	SDRunoPlugin_1090Form::show_validPreambles	(int n) {
	validPreambles. caption (std::to_string (n));
}

void	SDRunoPlugin_1090Form::show_goodCrc		(int n) {
	goodCrc. caption  (std::to_string (n));
}

void	SDRunoPlugin_1090Form::show_nrfixed		(int n) {
	nrfixed. caption (std::to_string (n));
}

void	SDRunoPlugin_1090Form::show_single_bit_fixed	(int n) {
	single_bit_fixed. caption (std::to_string (n));
}

void	SDRunoPlugin_1090Form::show_two_bit_fixed	(int n) {
	two_bit_fixed. caption (std::to_string (n));
}

void	SDRunoPlugin_1090Form::show_text	        (const std::string &s) {
	text_1090. caption (s);
}

void	SDRunoPlugin_1090Form::set_correction		(const std::string &s) {
	m_parent. set_correction (s);
}

void	SDRunoPlugin_1090Form::set_displayMode		(const std::string &s) {
	m_parent. set_displayMode (s);
}

void	SDRunoPlugin_1090Form::set_metricsMode		(const std::string &s) {
