...
//
std::string aircraft::showPlane	(bool metric, time_t now) {
...
//	Convert units to metric if --metric was specified. */
	...
	char t [256];
	sprintf (t, "%-6s %-8s %-9d %-7d %-7.03f   %-7.03f   %-3d   %-9ld %d sec\n",
	        hexaddr, flight, altitude, speed,
	        lat, lon, track, messages, (int)(now - seen));
	return std::string (t);
}

std::string	showPlanes (aircraft *aircrafts, bool metric) {
int amount	= 15;
int	count = 0;
time_t now	= time (NULL);
char progress [4];
